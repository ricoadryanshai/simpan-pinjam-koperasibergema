...
export const PinjamDetailModal = (props) => {
  ...
  const fetchPinjamData = async (kodeAnggota) => {
    if (kodeAnggota === selectedRow?.kodeAnggota) {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          console.log("Error fungsi fetchPinjamData di PinjamDetailModal.jsx");
        }
      } catch (error) {
      ...
      console.log("kodeAnggota tidak cocok dengan selectedRow.kodeAnggota");
      // Handle mismatch of kodeAnggota and selectedRow.kodeAnggota
    }
  };
  ...
  const currentEntries = pinjamDetail.slice(
    indexOfFirstEntry,
    indexOfLastEntry
  );
      ...
      <Modal.Body>
            ...
            {currentEntries
              ...
              .map((detail, index) => (
                <tr key={detail.id} className="text-center">
                  ...
                  <td>{detail && detail.angsuran ? detail.angsuran : ""}</td>
                  <td
        ...
        <div className="d-flex justify-content-center mt-2">
          <Pagination>
            ...
            {[...Array(Math.ceil(pinjamDetail.length / ITEMS_PER_PAGE))].map(
              (_, index) => (
                <Pagination.Item
                  key={index + 1}
